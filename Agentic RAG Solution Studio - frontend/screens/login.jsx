// Login — authenticates against backend DB
const Login = ({ onLogin }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    setLoading(true);
    try {
      const user = await API.auth.login(studioId.trim(), password);
      onLogin(user);
    } catch (err) {
      setLoading(false);
      setError(err.message || "Invalid Studio ID or password.");
      setShake(true);
      ...
      <div style={{ position: "relative", zIndex: 1, width: "100%", maxWidth: 420 }}>
        <div style={{ textAlign: "center", marginBottom: 40 }}>
          <div style={{ width: 56, height: 56, borderRadius: 16, background: "var(--bg-card)",
            border: "1px solid var(--acc-lime-line)", display: "inline-flex",
          ...
          <div style={{ fontSize: 12.5, color: "var(--text-muted)", marginTop: 6, letterSpacing: "0.04em" }}>
            Enterprise RAG Solution Builder · v0.1.0
          </div>
        </div>

        <div className="card" style={{ padding: "32px 36px", border: "1px solid var(--line-soft)",
          ...
          {error && (
            <div style={{ padding: "10px 14px", borderRadius: 8, marginBottom: 18,
              ...
              border: "1px solid var(--acc-red, #e55)", color: "var(--acc-red, #e55)",
          ...
          <form onSubmit={handleSubmit}>
            <div style={{ marginBottom: 16 }}>
              <label style={{ display: "block", fontSize: 11.5, fontWeight: 600,
                ...
                color: "var(--text-muted)", marginBottom: 6 }}>Studio ID</label>
              <input className="input" type="text" placeholder="e.g. ADMIN101"
                value={studioId}
            ...
            <div style={{ marginBottom: 28 }}>
              <label style={{ display: "block", fontSize: 11.5, fontWeight: 600,
                textTransform: "uppercase", letterSpacing: "0.07em",
                color: "var(--text-muted)", marginBottom: 6 }}>Password</label>
              <div style={{ position: "relative" }}>
                <input className="input" type={showPass ? "text" : "password"}
                  placeholder="••••••••" value={password}
                  onChange={e => { setPassword(e.target.value); setError(null); }}
                  autoComplete="current-password" style={{ width: "100%", paddingRight: 42 }} />
